...
@view_routes.route('/view_data', methods=['GET', 'POST'])
def view_data():
    conn = None
    try:
        ...
        if request.method == 'POST':
            ...
            selected_products = request.form.getlist('product_name')
        ...
        query = """
            SELECT * FROM dbo.ProductMovementSummary
        ...
        if selected_products:
            query += " AND Product_Name IN ({})".format(','.join(['?'] * len(selected_products)))
        ...
        cursor.execute("SELECT DISTINCT Product_Name FROM dbo.Product_Table")
        products = [row[0] for row in cursor.fetchall()]

        return render_template('view_data.html', data=data, columns=column_names, start_date=start_date.strftime('%Y-%m-%d'), end_date=end_date.strftime('%Y-%m-%d'), mills=mills, products=products, selected_mills=selected_mills, selected_products=selected_products)
...
@view_routes.route('/export_data', methods=['GET'])
def export_data():
    conn = None
    try:
        conn = create_connection()
        ...
        selected_products = request.args.getlist('product_name')
        ...
        query = """
            SELECT Date, Mill_Name, Product_Name, Opening_Balance, Closing_Balance, Movement
            FROM dbo.ProductMovementSummary
            WHERE Date BETWEEN ? AND ?
        """
        params = [start_date, end_date]
